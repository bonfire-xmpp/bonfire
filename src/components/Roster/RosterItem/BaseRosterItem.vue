<template>
  <div class="flex">
    ...
    <span class="gutter indicator-light border-circle-right flex-shrink-0" :class="selected ? 'bg-grey-800' : ''"/>

    <!-- Roster item card  -->
    <div class="flex-grow hide-overflow flex rounded main" :aria-selected="selected" >
        <!-- Wrap everything except vertical ... into a NuxtLink -->
        <router-link :to="`/chat/${item.jid}`" class="reset-link flex-grow flex hide-overflow">
          ...
          <div class="main-container ml-2 pr-2 flex-grow my-auto hide-overflow">
            <!-- Online status icon, username@domain -->
            <span>
              <span :class="selected ? '' : 'text-grey-700'">
                {{name}}
              </span>

              <span style="margin-left: -2px; margin-right: -2px;">
                <icon class="no-transition" :color="selected ? 'white' : 'grey-400'" size="1em">mdi-at</icon>
              </span><span :class="selected ? '' : 'text-grey-400'">
                {{domain}}
              </span>
            </span>
            ...
            <span class="text-grey-700" style="font-size: 12px;">{{statusMessage}}</span>
          </div>
        </router-link>
...
<script>
  ...
  export default {
    name: "RosterItem",
    components: { BadgedAvatar, RouterLink, TypingSpinner, Icon },
    props: {
      item: {
        type: Object,
        required: true,
