<template>
  <v-card flat outlined color="grey-400" class="unselectable"
          :disabled="disabled"
          :style="'--xep-animation-delay: ' + getDelay(delay) + 'ms'"
          :class="supported ? (animated ? 'blink-online' : 'supported') : 'unsupported'">
    <v-card-title class="text-white py-2 inline-block w-100">
    ...
    <v-card v-if="warning" flat color="warning" class="m-2">
      <v-card-title class="py-2 inline-block title-text"><icon size="1.2em" left>mdi-alert-circle-outline</icon>Warning!</v-card-title>
      <v-divider class="mb-n2 mt-n1"/>
