...
import { populateSearchIndex } from "@/assets/search";
...
async function insertBlock(messages, jid) {
    const timestamp = messages.reduce((acc, x) => Math.min(acc, x.timestamp), messages[0].timestamp);
    const compblock = lz4.compress(Buffer.from(msgpack.encode(messages)));
    const id = await messageDb.messageArchive.add({ block: compblock, timestamp, with: jid });
...
export const actions = {
    ...
    async [$actions.syncMessages] ( { dispatch }, jid ) {
        ...
        await delayIterator(async () => {
            ...
            const history = await this.$stanza.client.searchHistory({
                with: jid,
                paging: { before: lastID },
                end: lastTimestamp,
            });

            for (const { item: { message, delay } } of history.results) {
                if (message.body) {
...
export const mutations = {
    ...
    [$mutations.setMessages] ( state, { jid, messages }) {
        state[$states.messages][jid] = messages;
    },
    ...
    [$mutations.addMessage] ( state, { bareJid, message, messageState } ) {
        if(!state[$states.messages][bareJid])
            state[$states.messages][bareJid] = [];

        state[$states.messages][bareJid].push(message);

        state[$states.messagesById][message.id] = message;

        if(messageState) {
            state[$states.messageStateById][message.id] = messageState;
        }
    },

    [$mutations.updateMessageState] (state, { id, state: messageState } ) {
        state[$states.messageStateById][id] = messageState;

        messageDb.messageStates.put({id, ...messageState});
    },

    [$mutations.setComposing] (state, { user, composing }) {
        state[$states.chatComposing][user] = composing;
    },
};
