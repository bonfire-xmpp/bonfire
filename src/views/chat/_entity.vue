<template>
  <div class="flex flex-col bg-grey-200">
      ...
      <div class="py-2">
        ...
        <template v-else>
          <v-btn icon class="mx-2" ref="searchBar" @click="$refs.mobileDialog.open()">
            <icon size="1.66em" color="white">mdi-dots-vertical</icon>
          </v-btn>
    ...
    <!-- Main Section -->
    <main class="flex flex-row flex-grow hide-overflow">
      <div class="flex flex-col flex-grow">
        <!-- Message List -->
        <simplebar
          class="simplebar-no-gutter wide-scrollbar flex-grow"
        ...
        <div>
          <chat-message-form
            ...
            <template v-if="isTyping">
              ...
              <b class="text-grey-800">{{localPart(bare)}} is typing...</b>
            </template>
          </chat-message-form>
        </div>
      </div>
...
import {search, searchBlock} from "@/assets/search";
...
export default {
  ...
  computed: {
    ...
    messages () {
      const curblock = this.$store.state[MessageStore.namespace][MessageStore.$states.messages][this.entity];
  ...
  methods: {
    ...
    async search () {
      ...
      for (const block of blocks) {
        for (const msg of searchBlock(block, this.searchText)) {
          matches.push(msg);
        }
      }
    ...
    async fetchMessages () {
      ...
      const blocks = (await messageDb.messageArchive
