<template>
  <div class="main lhs-bg w-100 h-100" v-if="!$device.isMobileOrTablet">
    ...
    <div class="menu">
      ...
      <!--X exit button-->
      <button @click="$router.back();" class="exit-button rounded-full ring-grey-700 ring-1 focus:ring-2 p-1 hover:bg-grey-200">
        <icon size="1.33rem" color="white">mdi-close</icon>
      </button>

      <!--List of submenus-->
      <simplebar class="narrow-scrollbar" data-simplebar-auto-hide="true" data-simplebar-force-visible="false">
        <settings-menu-list class="narrow-menu" :menu-list="menuList" v-model:selected="selectedSubmenu"/>
      </simplebar>
    </div>
...
export default {
  ...
  components: {Icon, SettingsMenuList, Profile, About, Privacy, XEPs},
  data() {
    return {
      menuList: MenuList,
