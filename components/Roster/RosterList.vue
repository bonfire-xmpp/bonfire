  ...
  export default {
      ...
      jidLocations() {
        ...
        for (let i = 0; i < this.items?.length || 0; i++) {
          for (let j = 0; j < this.items[i].items?.length || 0; j++) {
            locations.set(JID.toBare(this.items[i].items[j].jid), [i, j]);
          }
        }
    ...
    watch: {
      selectedJid: {
        immediate: true,
        handler(jid) {
          const location = this.jidLocations.get(JID.toBare(jid));
          if(!location) return;
      ...
      jidLocations: {
        ...
        handler(jidLocations) {
          const location = jidLocations.get(JID.toBare(this.selectedJid));
          if(!location) return;
